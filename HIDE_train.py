...
# Zmiana importu datasetu na własny dataset HIDE
from dataset_HIDE import HIDEDataset  # Zakładam, że Twoja klasa nazywa się HIDEDataset

# Usunięcie niepotrzebnego importu, który odnosi się do modelu, którego nie używamy
from fftformer import fftformer  # Zakładam, że importujesz tylko fftformer

# Konfiguracja SSL, jeśli potrzebna do pobierania danych przez HTTPS
...
def train(train_loader, model, optimizer, epoch, device):
    ...
    criterion = torch.nn.MSELoss()  # Można rozważyć dodanie innych metryk jeśli jest to potrzebne
    ssim = SSIM(data_range=1.0)
    with tqdm(train_loader) as _tqdm:
        for x, y in _tqdm:  # Zakładam, że dataset zwraca tylko parę x (wejście) i y (oczekiwany wynik)
            x = x.to(device)
            y = y.to(device)
            ...
            loss = mse_loss  # Możliwość kombinowania struktury straty jak skomentowane
            cur_loss = loss.item()
...
def validate(validate_loader, model, epoch, device):
    ...
    with torch.no_grad():
        with tqdm(validate_loader) as _tqdm:
            for x, y in _tqdm:  # Usunięcie zmiennej 'z', zakładając, że nie jest potrzebna
                x = x.to(device)
                y = y.to(device)
...
def initial_values(validate_loader, device):
    ...
    with torch.no_grad():
        with tqdm(validate_loader) as _tqdm:
            for x, y in _tqdm:  # Usuwam 'z', zakładając że nie jest używane
                x = x.to(device)
                y = y.to(device)

                # Oblicz strate jako różnicę między obrazem wejściowym a oczekiwanym wynikiem
...
def main():
    args = get_args()
    batch_size = 1  # Zwiększone dla efektywności, dostosuj w zależności od możliwości GPU
    num_workers = 8
    learning_rate = 0.0001
    lr_decay_rate = 0.5
    lr_decay_step = 10
    epochs = 50
    img_size = (1280, 720)  # Zaktualizowany rozmiar zgodny z rozmiarem obrazów w dataset HIDE
